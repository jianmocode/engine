...
class ComponentParser {
	...
	function load( $template ) {
		...
		return $this->loadComponent( file_get_contents($template) );
	}
	...
	function loadComponent( $template ) {
		...
		$this->template =preg_replace("/<\/html>/", "", $this->template);
		$this->template =preg_replace("/<body[^>]*>/", "", $this->template);
		$this->template =preg_replace("/<\/body>/", "", $this->template);
		$this->template =preg_replace("/<head>[^>]*>/", "", $this->template);
		$this->template =preg_replace("/<\/head>/", "", $this->template);
	...
	function toPHP() {
		...
		$this->compile();
		$node = $this->dom->getElementsByTagName('body')->item(0);
 	...
 	// 	echo "== DEBUG ====\n";
		...
		$body = empty($body[1]) ? $html : $body[1];

		$body = str_replace('<assets>', '', $body );
		$body = str_replace('</assets>', '', $body );
	 ...
	 * @return [type] [description]
	 */
	function insertAssets( $assets = [], $pos="assets" ) {
		...
		$firstSibling = $elms->parentNode->firstChild;
		...
		foreach ( $assets as $file ) {
			if ( substr($file, -3) == 'css' ) {
				...
				$elm->insertBefore( $style, $firstSibling );
				...
				$elm->insertBefore( $script , $firstSibling);
			}
		}
