...
class TextParser {
	...
	function load( $template ) {
		...
		return $this->loadText( file_get_contents($template) );
	}
	...
	function loadText( $template ) {
		...
		$this->template = '<html <head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"></meta>><body>' . $template . "</body></html>";
	...
	function toPHP() {
		...
		$bodys = $this->dom->getElementsByTagName('body');
		foreach ($bodys as $body) {}
		$html = $this->dom->saveHTML($body->firstChild);
