...
class apiv1UserController extends apiController {
	 ...
	 */
	function find() {
		$page = ( isset($this->query['page']) && !empty($this->query['page']) ) ? $this->query['page'] : 1;
		$user = M('User');
		$resp = $user->query()->paginate(50, ['*'], '', $page )->toArray();
		echo json_encode( ['code'=>0, 'resp'=>$resp] );
	}
