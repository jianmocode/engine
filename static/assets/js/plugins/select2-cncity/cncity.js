...
CNCity.prototype.find	= function(id) {
	if(typeof(this.items[id]) == "undefined")
		return false;
	return this.items[id];
}

CNCity.prototype.fillOption	= function(el_id , loc_id , selected_id) {
	// var el	= $('#'+el_id); 
	// console.log( $(el_id).attr('name'), $(el_id).attr('lc-type') );
	var el = $(el_id);
	...
	if (json) {
		...
		$.each(json , function(k , v) {
			...
			// 	console.log("selected_index", selected_index, selected_id);
			// }
...
CNCity.prototype.val = function( name, value  ) {
	
	value = value || null;

	var elm = $('[name='+name+']') || $('#' + name);
	...
	if ( value != null ) {
		...
		var province = $("[lc-type=province]", par);
		var city = $("[lc-type=city]", par);
		var town = $("[lc-type=town]", par);

		var arr = value.split(',');
		if ( arr[1] != undefined ) {
			// console.log(arr[1], $(province).attr('name') );
			this.fillOption(province , '0', arr[1] );
			$(province).change();

			if ( arr[2] != undefined ) {
				// console.log(arr[2], $(city).attr('name') );
				this.fillOption(city , '0,'+ arr[1] + ",", arr[2]);
				$(city).change();

				if ( arr[3] != undefined ) {
					// console.log(arr[3], $(town).attr('name') );
					this.fillOption(town , '0,'+ arr[1] + "," + arr[2] + "," , arr[3]);
					$(town).change();
				}
			}
		}

		// console.log(value, province, $('option', province));
		// this.fillOption(province , value);
		return;
	}



	result = {province:"省份", city: "城市", town:"区县", code:""};
	if ( val == undefined ){
		return result;
	}
	...
	if ( arr[1] != undefined ) {
		...
		result['province'] = this.items["0"][p];
	...
	if ( arr[3] != undefined ) {
		var p = parseInt(arr[1]);
		var c = parseInt(arr[2]);
		var t = parseInt(arr[3]);
		result['town'] = this.items["0," + p + "," + c][t];
...
CNCity.prototype.init = function( elm ) {

	var loc = this;

	elm = elm || '.select2-cncity';
	var lcgroup = {};
	...
	$(elm).each(function(idx, el ){
		...
		var t = $(el).attr('lc-type');

		if ( typeof lcgroup[n] == 'undefined') {
			lcgroup[n] = {};
		}
	...
	for ( var name in lcgroup ) {
		var lc = lcgroup[name];

		var province = lc['province'];
		var city = lc['city'];	
		var town = lc['town'];
		if ( province != null ) {
			...
			if ( city != null ) {
				...
				$(province).change(function() {
					...
					var city = $("[lc-type=city]", par);
					var name = city.attr('name');
					city.empty();
					city.append(title[1]);
					loc.fillOption(city , '0,'+$(this).val());
					city.change();
					$('[name='+name+']').attr('data-value', '0,'+$(this).val() );
				...
				if ( town != null) {
					...
					$(city).change(function() {
						...
						$('[name='+name+']').attr('data-value', '0,' + province.val() + ',' + city.val())
					})
					...
					$(town).change(function(){
						...
						$('[name='+name+']').attr('data-value',  '0,' + province.val() + ',' + city.val() + ',' + town.val());
					})
				}
			}

			var value = $('[name='+name+']').attr('data-value');
			if ( value != "" ) {
				loc.val(name, value);
			} else {
				loc.val(name, "0,");
